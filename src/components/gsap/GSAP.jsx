	...
	return (
		<div className='row'>
			<div className='col-md-12 my-screen'>
				...
				<Canvas camera={{ position: [0, 0, 1], rotation: [0, 0, 0] }}>
					...
					>
						<Scroll html>
							<div className='marco' />
							<div className='marco'>
								<h3 style={{ color: 'white' }}>marcoPolo</h3>
							</div>
							<div className='marco'>
				...
				{'MAÑANAVIERNES'.split('').map((letter, key) => {
					return <h5 key={key}>{letter}</h5>;
				})}
				{[...text].map((letter, key) => {
					// Resultado parecido
					return <h5 key={key}>{letter}</h5>; // De esta forma si que incluye el espacio (pero no ocupara)
				})}
